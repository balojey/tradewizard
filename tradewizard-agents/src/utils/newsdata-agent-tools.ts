...
export class ResponseFormatter {
  ...
  static sanitizeArticle(article: NewsArticle): NewsArticle {
    return {
      ...
      ai: article.ai ? {
        tags: article.ai.tags?.map(t => this.sanitizeString(t)).filter(t => t.length > 0),
        sentiment: article.ai.sentiment,
        sentimentStats: article.ai.sentimentStats,
        regions: article.ai.regions?.map(r => this.sanitizeString(r)).filter(r => r.length > 0),
        organizations: article.ai.organizations?.map(o => this.sanitizeString(o)).filter(o => o.length > 0),
        summary: article.ai.summary ? this.sanitizeString(article.ai.summary) : undefined,
      } : undefined,
      crypto: article.crypto ? {
        coins: article.crypto.coins?.map(c => this.sanitizeString(c)).filter(c => c.length > 0),
      } : undefined,
      market: article.market ? {
        symbols: article.market.symbols?.map(s => this.sanitizeString(s)).filter(s => s.length > 0),
      } : undefined,
    };
  }
...
export class LatestNewsTool implements BaseNewsTool {
  ...
  private transformLatestParams(params: LatestNewsToolParams): LatestNewsParams {
    return {
      ...
      size: params.size,
      full_content: params.fullContent ? 1 : 0,
