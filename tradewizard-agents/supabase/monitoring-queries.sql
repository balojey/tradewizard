-- ============================================================================
-- TradeWizard Automated Market Monitor - Dashboard Queries
-- ============================================================================
-- SQL queries for monitoring market analysis statistics, agent performance,
-- cost tracking, and quota usage trends in Supabase
-- ============================================================================
...
-- QUOTA USAGE TRENDS
-- ============================================================================
-- Note: These queries assume quota tracking data is stored in analysis metadata
-- If quota data is stored separately, these queries will need adjustment
...
SELECT
  ...
  3 as configured_max_per_day, -- Default from requirements
  ROUND(
    100.0 * COUNT(ah.id) / 3,
    2
...
FROM analysis_history ah
WHERE ah.analysis_type IN ('initial', 'update') -- Exclude manual triggers
GROUP BY DATE(ah.created_at)
ORDER BY analysis_date DESC;
...
-- SELECT * FROM v_daily_analysis_stats WHERE analysis_date >= CURRENT_DATE - INTERVAL '30 days';

-- Get agent performance summary
-- SELECT * FROM v_agent_performance_summary;

-- Get cost tracking for current month
-- SELECT * FROM v_daily_cost_tracking WHERE cost_date >= DATE_TRUNC('month', CURRENT_DATE);

-- Get analysis stats for last 7 days
